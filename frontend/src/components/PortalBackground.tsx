...
const PortalBackground: React.FC<Props> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const rays: GodRay[] = Array.from({ length: NUM_RAYS }).map(() => {
      const isTop = Math.random() > 0.5;
      return {
        x: isTop ? Math.random() * W : -100, // Top edge or Left edge
        y: isTop ? -100 : Math.random() * H,
        width: Math.random() * 100 + 20, // Some wide, some narrow
