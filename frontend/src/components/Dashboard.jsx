...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/v1/crawls').then(res => res.json()).then(data => setJobs(data.jobs || [])).catch(console.error);
  ...
  const handleExport = () => {
    ...
    const csvContent = [headers.join(','), ...entities.map(e => `"${e.id}","${e.title}","${e.url}","${e.content_snippet.replace(/"/g, '""')}"`)].join('\n');
  ...
  return <div>
      ...
      gridTemplateColumns: '1fr 1fr'
        ...
        flexDirection: 'column'
          ...
          flex: 1
            ...
            <table className="data-table">
              ...
              <tbody>
                {currentJobs.map(job => <tr key={job.id}>
                    <td>{job.url}</td>
                    <td>
                ...
                {currentJobs.length === 0 && <tr>
                    <td colSpan={2} style={{
                  textAlign: 'center',
                  opacity: 0.7
                }}>No jobs found in current cycle.</td>
                  </tr>}
              </tbody>
            </table>
          </div>
          {totalJobsPages > 1 && <div className="pagination-controls">
              ...
              <button onClick={() => setJobsPage(p => Math.min(totalJobsPages, p + 1))} disabled={jobsPage === totalJobsPages}>
                Next
              </button>
            </div>}
        </div>
        ...
        flexDirection: 'column'
          ...
          <h3 style={{
          marginTop: 0
        }}>Recently Extracted Entities (with AI/NLP)</h3>
          <div className="data-table-wrapper" style={{
          flex: 1
        }}>
            <table className="data-table">
              ...
              <tbody>
                {currentEntities.map(entity => <tr key={entity.id}>
                    <td>
                ...
                {currentEntities.length === 0 && <tr>
                    <td colSpan={3} style={{
                  textAlign: 'center',
                  opacity: 0.7
                }}>No entities extracted yet.</td>
                  </tr>}
              </tbody>
            </table>
          </div>
          {totalEntitiesPages > 1 && <div className="pagination-controls">
              <button onClick={() => setEntitiesPage(p => Math.max(1, p - 1))} disabled={entitiesPage === 1}>
                Previous
              </button>
              <span className="pagination-info">Page {entitiesPage} of {totalEntitiesPages}</span>
              <button onClick={() => setEntitiesPage(p => Math.min(totalEntitiesPages, p + 1))} disabled={entitiesPage === totalEntitiesPages}>
                Next
              </button>
            </div>}
        </div>
