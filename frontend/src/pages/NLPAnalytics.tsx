...
export default function NLPAnalytics() {
  ...
  const sentimentData = [
    { name: 'Positive', value: positive, color: '#a3e635' },
    { name: 'Neutral', value: neutral, color: '#d4af37' },
  ...
  entities.forEach(e => {
    if (e.named_entities) {
      try {
        const parsed = JSON.parse(e.named_entities) as string[];
        parsed.forEach(name => {
      ...
      <div className="grid-cards">
        <div className="carved-panel kpi-card">
          ...
          {sentimentData.length > 0 ? (
            ...
            <p style={{ opacity: 0.7, marginTop: '20px' }}>No sentiment data available.</p>
          )}
        </div>
        ...
        <div className="carved-panel kpi-card">
          ...
          {topEntities.length > 0 ? (
            ...
            <p style={{ opacity: 0.7, marginTop: '20px' }}>No entities extracted yet.</p>
          )}
        </div>
      </div>

      <div className="carved-panel" style={{ marginTop: '24px' }}>
            ...
            <tbody>
              {entities.map(entity => {
                ...
                return (
                  <tr key={entity.id}>
                    <td>
                      <a href={entity.url} target="_blank" rel="noreferrer" style={{ color: 'var(--text-h)', textDecoration: 'none', fontWeight: 'bold' }}>
                        {entity.title}
                      </a>
                    </td>
                    <td>
                      {entity.sentiment_label ? (
                        ...
                        <span className="status-badge" style={{ opacity: 0.5 }}>Pending Analysis</span>
                      )}
                    </td>
                    <td>
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        {Object.entries(parsedData).map(([k, v]) => {
                          if (!v) return null;
                          return (
                            <span key={k} style={{ fontSize: '0.8rem', background: 'var(--code-bg)', padding: '2px 6px', borderRadius: '4px', border: '1px solid var(--border)' }}>
                              <strong style={{ color: 'var(--accent-light)' }}>{k}:</strong> {v as string}
                            </span>
                          )
                        })}
                      </div>
              ...
              {entities.length === 0 && (
                <tr>
                  <td colSpan={3} style={{ textAlign: 'center', opacity: 0.7 }}>No semantic data available.</td>
                </tr>
              )}
            </tbody>
          </table>
