...
export default function Alerts() {
  ...
  const fetchAlerts = () => {
    fetch('http://localhost:8000/api/v1/alerts').then(res => res.json()).then(data => {
      setRules(data.rules || []);
  ...
  const handleSubmit = async e => {
    ...
    await fetch('http://localhost:8000/api/v1/alerts', {
      ...
      body: JSON.stringify({
        name,
        keyword,
        sentiment_threshold: sentimentThreshold || null
      })
    });
    setName('');
    setKeyword('');
